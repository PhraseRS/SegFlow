<p align="center">
  <a href="ENVIRONMENT_REQUIREMENTS.md">English</a> | <strong>简体中文</strong>
</p>
...
SegFlow 的依赖体系天然存在**两个独立层次**，设计上已有一定的解耦意识（体现在 `try/except ImportError` 的可选导入模式），但尚未形成清晰的文档化分层。

```
┌────────────────────────────────────────────────────────────┐
│                    Layer A：GUI 宿主环境                    │
│  (运行 SegFlow 本体所需，必须在启动 Python 环境中安装)      │
│                                                            │
...
| `CUDA Toolkit` | 与 PyTorch 版本匹配 | GPU 加速（训练/推理必需） |
...
| **conda 非强制** | GUI 本体不强依赖 conda（已有容错），但训练/推理的 GPU 环境必须使用 conda 管理 |
| **需要 GPU** | 训练/推理环境为 GPU 环境，需安装与目标 CUDA 版本兼容的 NVIDIA 驱动；GUI 本体的数据管理/可视化功能不需要 GPU，可独立使用 |
